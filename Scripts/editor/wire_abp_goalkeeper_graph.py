"""Wire the AnimGraph of /Game/Blueprints/Player/ABP_Goalkeeper headlessly.

Adds (or updates) a UAnimGraphNode_MotionMatching driving the inherited Output
Pose, with its Database property set to MMDB_Goalkeeper. Compiles and saves.
...
Python directly. The helper is database-agnostic (same one used for the
outfield AnimBP).

Idempotent: re-running won't duplicate the MM node.

NOTE: MMDB_Goalkeeper is empty at the time this runs — that's intentional.
The BP compiles cleanly against an empty database; it just produces a T-pose
at runtime until Mixamo GK clips are added to the database.
"""

import sys
import unreal

ANIMBP_PATH = "/Game/Blueprints/Player/ABP_Goalkeeper"
DATABASE_PATH = "/Game/Animation/MotionMatching/MMDB_Goalkeeper"
LOG_PATH = "/tmp/wire_abp_goalkeeper_graph.log"
